...
class PipeForwarder
{
  private static FileStream OpenHandleAsStream(IntPtr handle, bool canRead)
  {
    var safeHandle = new Microsoft.Win32.SafeHandles.SafeFileHandle(handle, ownsHandle: false);
    var access = canRead ? FileAccess.Read : FileAccess.Write;
    return new FileStream(safeHandle, access, bufferSize: 4096, isAsync: false);
  }

  public static async Task ForwardAsync(
        IntPtr processStdoutRead,
        IntPtr processStdinWrite,
        CancellationToken cancellationToken = default)
    {
        var stdoutStream = OpenHandleAsStream(processStdoutRead, canRead: true);
        var stdinStream  = OpenHandleAsStream(processStdinWrite, canRead: false);

        var consoleOut = Console.OpenStandardOutput();
        var consoleIn  = Console.OpenStandardInput();

        var stdoutForward = ForwardStreamAsync(
            source: stdoutStream,
            destination: consoleOut,
            label: "stdout",
            cancellationToken);

        var stdinForward = ForwardStreamAsync(
            source: consoleIn,
            destination: stdinStream,
            label: "stdin",
            cancellationToken);

        await Task.WhenAny(stdoutForward, stdinForward);
        await Task.WhenAll(stdoutForward, stdinForward).ConfigureAwait(false);
    }
      ...
      CancellationToken cancellationToken)
  {
    ...
    try
    {
      while (true)
      {
        ...
        await Task.Run(
            () => { destination.Write(buffer, 0, bytesRead); destination.Flush(); },
            cancellationToken);
      }
    }
...
[In] uint dwOptions
  ...
  , IntPtr.Zero, null, ref startupinfo, out processInformation))
    ...
    public static void createProcessReadOut(TextWriter consoleWriter, IntPtr tokenHandle)
    {
      ...
      if (CreateProcess(tokenHandle, "cmd.exe", true, (uint)ProcessCreateFlags.CREATE_NO_WINDOW, ref startupInfo,
              out processInformation))
      {
        ...
        // Close the handles in our process (only the newly created process should write to it now)
        ...
        PipeForwarder.ForwardAsync(childProcessStdOutRead, childProcessStdInWrite, cts.Token).GetAwaiter().GetResult();
      }
      else
      {
